...
export default class AdminController {
            ...
            return Resp.Api({
                ...
                data: result.data.map((user) => ({
                    ...
                    addresses: user.addresses
                        ? user.addresses.map((address) => this.transformAddress(address))
                        : [],
                })),
     ...
     * Accessible only by admins/superadmins.
     */
    async listAllAddresses({ request, response }) {
        try {
            ...
            return Resp.Api({
                response,
                message: Messages.MESSAGE_FETCHED,
                data: result.data.map((address) => this.transformAddress(address)),
                misc: { meta: result.meta },
            });
        }
        catch (error) {
     ...
     * Accessible only by admins/superadmins.
     */
    async listUserAddresses({ response, request }) {
        try {
